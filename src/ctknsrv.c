//
//  Auth Exits sample token server for LightWave Client.
//
#pragma nolist
...
static void handleFetchTknSetRequest(CTKNSRV_FUNCTION_ARGS);

//  The dispatch table.
typedef struct _dispatchEntry {
  short msgCode;
  void (*functionRef)(CTKNSRV_FUNCTION_ARGS);
} dispatchEntry;
...
#define IO_SIZE 2097152 /* Max I/O size - 2MB */
  ...
  logMessage("CLIENT Token Server starting");
...
static void handleFetchTknSetRequest(CTKNSRV_FUNCTION_ARGS) {
  ...
  if (rq->tkn_set_name_len <= 0 || // Default is token set 1
    ...
    rp.tkn_set[0].base64_encode = 1;
    strncpy(rp.tkn_set[0].tkn_format, "header-token-1-%s",
    ...
    rp.tkn_set[0].tkn_value_len =
        snprintf(rp.tkn_set[0].tkn_value, sizeof(rp.tkn_set[0].tkn_value),
                 "testae-header-token-%lld", rq->header.rq_ts_unique[0]);
    ...
    strncpy(rp.tkn_set[0].tkn_format, "header-token-2-%s",
            sizeof(rp.tkn_set[0].tkn_format));
    rp.tkn_set[0].tkn_format_len = (int)strlen(rp.tkn_set[0].tkn_format);
    strncpy(rp.tkn_set[0].tkn_name, "Authorization",
            sizeof(rp.tkn_set[0].tkn_name));
    ...
    rp.tkn_set[1].tkn_value_len =
        snprintf(rp.tkn_set[1].tkn_value, sizeof(rp.tkn_set[1].tkn_value),
                 "testae-param-token-%lld", rq->header.rq_ts_unique[1]);
    ...
    rp.tkn_set[0].tkn_value_len =
        snprintf(rp.tkn_set[0].tkn_value, sizeof(rp.tkn_set[0].tkn_value),
                 "testae-param-token-%lld", rq->header.rq_ts_unique[0]);
